// net.zig - Adria Permissioned Ledger (MPL) Networking
...
pub const DISCOVERY_MAGIC = [4]u8{ 0xDE, 0x11, 0xC0, 0x1E }; // APL discovery magic
...
pub const Peer = struct {
    ...
    pub fn connect(self: *Peer) !void {
        ...
        self.socket = net.tcpConnectToAddress(addr) catch |err| {
            ...
            // Suppress error for bootstrap nodes as they might be offline in dev
            std.debug.print("[WARN] Bootstrap node {s} offline/unreachable: {}\n", .{ addr_str, err });
            return err;
        };
        ...
        var addr_buf: [32]u8 = undefined;
        const addr_str = self.address.toString(&addr_buf);
        std.debug.print("[INFO] Connecting to {s} (attempt #{})\n", .{ addr_str, self.connection_attempts });
    }
    ...
    pub fn disconnect(self: *Peer) void {
        ...
        self.state = .disconnected;
        std.debug.print("[INFO] Disconnected from peer {any}\n", .{self.address});
    }
    ...
    pub fn sendVersion(self: *Peer, blockchain_height: u32) !void {
        ...
        std.debug.print("[INFO] Sent version to peer\n", .{});
    }
    ...
    pub fn sendPing(self: *Peer) !void {
        ...
        std.debug.print("[INFO] Ping sent to peer\n", .{});
    }
    ...
    pub fn broadcastTransaction(self: *Peer, tx: types.Transaction) !void {
        ...
        std.debug.print("[INFO] Broadcasted transaction to peer\n", .{});
    }
    ...
    pub fn broadcastBlock(self: *Peer, block: types.Block) !void {
        ...
        std.debug.print("[INFO] Broadcasted block to peer\n", .{});
    }
};
...
pub const NetworkManager = struct {
    ...
    pub fn addPeer(self: *NetworkManager, address_str: []const u8) !void {
        ...
        std.debug.print("[INFO] Added peer: {s}\n", .{address_str});
    }
    ...
    pub fn broadcastTransaction(self: *NetworkManager, tx: types.Transaction) void {
        ...
        std.debug.print("[INFO] Transaction broadcasted to {} peers\n", .{self.peers.items.len});
    }

    pub fn broadcastBlock(self: *NetworkManager, block: types.Block) void {
        for (self.peers.items) |*peer| {
            peer.broadcastBlock(block) catch {
                std.debug.print("[WARN] Failed to broadcast block to peer\n", .{});
            };
        }
        std.debug.print("[INFO] Block broadcasted to {} peers\n", .{self.peers.items.len});
    }
    ...
    pub fn startClient(self: *NetworkManager, port: u16) !void {
        ...
        self.is_running = true;
        std.debug.print("[INFO] APL client network started (outbound connections only)\n", .{});
    }
    ...
    pub fn connectToPeer(self: *NetworkManager, address: NetworkAddress) !void {
        ...
        peer_ptr.connect() catch |err| {
            ...
            std.debug.print("[WARN] Failed to connect to peer {s}: {}\n", .{ addr_str, err });
            return err;
        };

        var addr_buf: [32]u8 = undefined;
        const addr_str = address.toString(&addr_buf);
        std.debug.print("[INFO] Connected to peer {s}\n", .{addr_str});
    }
    ...
    fn connectToBootstrapNodes(self: *NetworkManager) !void {
        ...
        if (std.process.getEnvVarOwned(self.allocator, "ADRIA_BOOTSTRAP")) |env_value| {
            defer self.allocator.free(env_value);

            std.debug.print("[INFO] Using bootstrap nodes from ADRIA_BOOTSTRAP: {s}\n", .{env_value});
            ...
            std.debug.print("[INFO] Using default bootstrap nodes\n", .{});
    ...
    fn listenForDiscoveryResponses(self: *NetworkManager, our_port: u16) !void {
        ...
        while (attempts < 10 and self.is_running) {
            const bytes_received = std.posix.recvfrom(socket, &buffer, 0, &sender_addr, &sender_len) catch |err| switch (err) {
                error.WouldBlock => {
                    ...
                    std.debug.print("[WARN] Discovery: receive error: {}\n", .{err});
                    attempts += 1;
        ...
        std.debug.print("[INFO] Discovery session complete\n", .{});
    }
    ...
    fn scanLocalNetwork(self: *NetworkManager, our_port: u16) !void {
        ...
        while (ip < 255) : (ip += 1) {
            ...
            if (self.testTCPConnection(peer_addr_str)) {
                std.debug.print("[INFO] Found peer via scan: {s}\n", .{peer_addr_str});
                self.addPeer(peer_addr_str) catch {};
            }
...
fn acceptConnections(network: *NetworkManager) void {
    const addr = network.listen_address orelse {
        std.debug.print("[ERROR] No listen address set\n", .{});
        return;
    };
    ...
    std.posix.bind(socket, &addr.any, @sizeOf(std.posix.sockaddr.in)) catch |err| {
        std.debug.print("[ERROR] Failed to bind: {}\n", .{err});
        return;
    };
    ...
    std.debug.print("[INFO] Server listening for connections on {}\n", .{addr});
        ...
        const client_socket = std.posix.accept(socket, &client_addr, &client_addr_len, 0) catch |err| switch (err) {
            error.WouldBlock => {
                ...
                if (network.is_running) {
                    std.debug.print("[WARN] Accept error: {}\n", .{err});
                }
                continue;
            },
        };

        std.debug.print("[INFO] New peer connected!\n", .{});

        // Handle this connection in a separate thread
        const thread = Thread.spawn(.{}, handleIncomingPeer, .{ network, client_socket }) catch |err| {
            std.debug.print("[WARN] Failed to spawn peer thread: {}\n", .{err});
    ...
    std.debug.print("[INFO] Accept thread stopped\n", .{});
}
...
fn handleIncomingPeer(network: *NetworkManager, client_socket: std.posix.socket_t) void {
    ...
    while (network.is_running) {
        ...
        const bytes_read = std.posix.recv(client_socket, &buffer, 0) catch |err| switch (err) {
            error.WouldBlock => {
                ...
                if (network.is_running) {
                    std.debug.print("[WARN] Read error from peer: {}\n", .{err});
                }
                break;
            },
        };
    ...
    std.debug.print("[INFO] Incoming peer disconnected\n", .{});
}
...
fn handlePeerConnection(network: *NetworkManager, stream: net.Stream) void {
    ...
    while (network.is_running) {
        ...
        const bytes_read = stream.read(&buffer) catch |err| switch (err) {
            ...
            error.WouldBlock => {
                ...
                std.debug.print("[WARN] Read error: {}\n", .{err});
                break;
            },
        };
    ...
    std.debug.print("[INFO] Peer disconnected\n", .{});
}
...
fn processMessage(network: *NetworkManager, data: []const u8) void {
    ...
    if (!std.mem.eql(u8, &header.magic, &MAGIC_BYTES)) {
        std.debug.print("[WARN] Invalid magic bytes\n", .{});
        return;
    }
    ...
    std.debug.print("[INFO] Received message: {s} ({}bytes)\n", .{ command, header.length });
    ...
    if (std.mem.eql(u8, command, "version")) {
        ...
        std.debug.print("[INFO] Block received from network\n", .{});
...
fn processIncomingBlock(network: *NetworkManager, block_data: []const u8) void {
    ...
    if (network.blockchain == null) {
        std.debug.print("[WARN] Block received but no blockchain connection\n", .{});
        return;
    }

    // Parse block data
    if (block_data.len < @sizeOf(types.BlockHeader) + @sizeOf(u32)) {
        std.debug.print("[WARN] Block too small - incomplete data\n", .{});
        return;
    }
    ...
    std.debug.print("[INFO] Block received: {} transactions, timestamp {}\n", .{ tx_count, header.timestamp });

    // Create transaction array
    const transactions = network.allocator.alloc(types.Transaction, tx_count) catch {
        std.debug.print("[ERROR] Memory allocation failed - block not processed\n", .{});
        return;
    };
    ...
    for (transactions, 0..) |*tx, i| {
        if (tx_offset + @sizeOf(types.Transaction) > block_data.len) {
            std.debug.print("[WARN] Transaction {} beyond data boundary\n", .{i});
            return;
        }
    ...
    if (network.blockchain) |blockchain| {
        blockchain.handleIncomingBlock(block) catch |err| {
            std.debug.print("[ERROR] Block rejected by blockchain: {}\n", .{err});
        };
    }
}
...
fn handleVersionMessage(network: *NetworkManager, version_data: []const u8) void {
    ...
    if (network.blockchain) |blockchain| {
        ...
        if (peer_version.block_height > our_height) {
            ...
            std.debug.print("[INFO] We are {} blocks behind, need to sync\n", .{blocks_behind});
            ...
            std.debug.print("[INFO] We are {} blocks ahead\n", .{blocks_ahead});
        } else {
            std.debug.print("[INFO] Heights are equal\n", .{});
        }
    }

    // TODO: Send verack response
    std.debug.print("[INFO] Handshake complete\n", .{});
}

// Handle ping message
fn handlePingMessage(network: *NetworkManager) void {
    _ = network; // TODO: Send pong back to specific peer
    std.debug.print("[INFO] Ping received - sending pong\n", .{});
...
fn handlePongMessage(network: *NetworkManager) void {
    ...
    std.debug.print("[INFO] Pong received - peer alive\n", .{});
    // TODO: Update peer last_seen timestamp
}

// Maintenance thread
fn maintainConnections(network: *NetworkManager) void {
    std.debug.print("[INFO] Connection maintenance active\n", .{});
    ...
    while (network.is_running) {
        ...
        for (network.peers.items) |*peer| {
            ...
            if (peer.state == .connected and time_since_seen > 60) { // 60 seconds silence
                std.debug.print("[WARN] Peer silent for 60s - checking health\n", .{});
                peer.sendPing() catch {
                    std.debug.print("[INFO] Peer connection lost, will reconnect\n", .{});
